...
```php
<?php
echo $name; // Primera forma: con echo
print $name; // Segunda forma: con print
?>

<?= $name ?> // Tercera forma: con sintaxis corta
```
...
```php
<?php
echo 'Es una cadena con una comilla simple: Isn\'t it?';
?>
```
...
<head>
 ...
 <meta http-equiv="expires" content="Sat, 07 Feb 2016 00:00:00 GMT"> <!-- Evita el caché del navegador -->
...
Para generar documentación HTML con phpDocumentor se debe utilizar el archivo [`phpDocumentor.phar`](https://github.com/phpDocumentor/phpDocumentor/releases), mediante el siguiente comando en la raíz del proyecto:

```bash
php phpDocumentor.phar -d . -t docs/api
```

![phpDocumentor](img/phpdoc.png)
...
```php
// Incluye el menú; si falta, PHP muestra una advertencia y continúa.
include 'menu.php';
// menu.php contiene:
// <nav><a href="index.php">Inicio</a> | <a href="contacto.php">Contacto</a></nav>
echo '<h1>Página de inicio</h1>';

// Incluye las funciones solo una vez, aunque se solicite varias veces.
include_once 'functions.php';
include_once 'functions.php';
// functions.php contiene:
// function formatTitle(string $title): string { return ucfirst(strtolower($title)); }
echo formatTitle('Inicio');

// Carga la configuración necesaria para que la aplicación pueda funcionar.
require 'config.php';
// config.php podría contener, por ejemplo:
// $config = ['appName' => 'Mi aplicación', 'debug' => true];
echo $config['appName'];

// Carga la conexión a la base de datos una sola vez.
require_once 'database.php';
// database.php contiene la conexión a la base de datos en $database.
...
Por ejemplo, una página puede reutilizar una cabecera y un pie comunes:
